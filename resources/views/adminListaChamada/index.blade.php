...
@section('title', 'Union Eleições - Lista de Chamada')
...
<div class="container">
    <div class="container-fluid">
        ...
        <div class="mb-5">
            ...
            <p class="text-muted mt-1">Lista de eleitores cadastrados no sistema para chamada.</p>
        </div>

        <!-- Filtro / Busca -->
        <div class="card shadow-lg border-0 rounded-3 stat-card mb-4 px-4 py-3">
            <form action="{{ route('admin.adminListaChamada.index') }}" method="GET" class="row g-2 align-items-center form-o">
                <div class="col-md-9">
                ...
                <div class="col-md-3 d-flex">
                    ...
                    <button type="button" class="btn botao-buscar me-2 w-100" title="Baixar Lista de Eleitores em PDF" data-bs-toggle="modal" data-bs-target="#gerarModalPdfListaDeChamada">
                        <i class="fa-solid fa-file-pdf"></i>
                    </button>
                    <a data-bs-toggle="modal" data-bs-target="#chamadaListaGerarModalExcel"title="Exportar Excel" class="btn botao-buscar w-100 me-3">
        ...
        <div class="modal fade" id="gerarModalPdfListaDeChamada" tabindex="-1" aria-labelledby="modalPdfVotantesLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg rounded-3">
                    <div class="modal-header text-white fund-cont">
                        <h5 class="modal-title m-auto" id="modalPdfVotantesLabel">Gerar PDF - Lista de Chamada</h5>
                    </div>
                    <div>
                        <form id="listaChamadaFormGerarPdf" target="_blank" action="{{ route('admin.adminListaChamada.gerarPdf') }}" method="POST">
                            @csrf
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="nome_arquivo" name="nome_arquivo"
                                        placeholder="Ex: lista_de_chamada">
                                </div>
                            </div>
        ...
        <!-- Modal Excel -->
        <div class="modal fade" id="chamadaListaGerarModalExcel" tabindex="-1" aria-labelledby="modalExcelLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg rounded-3">
                    <div class="modal-header text-white fund-cont">
                        <h5 class="modal-title m-auto" id="modalExcelLabel">Gerar Excel - Lista de Chamada</h5>
                    </div>
                    <form id="gerChamadaListaExcelForm" target="_blank" action="{{ route('admin.adminListaChamada.exportarExcel') }}" method="POST">
                        @csrf
                        <div class="modal-body">

                            <div class="mb-3">
                                <label for="nome_arquivo" class="form-label fw-semibold">Nome do arquivo:</label>
                                <input type="text" class="form-control" id="nome_arquivo" name="nome_arquivo" placeholder="Ex: lista_de_chamada">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label">Data e Horário</label>
                                </div>
                            </div>
            ...
            <div class="card-body p-3 p-md-5">
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($listaChamadas as $lista)
                                <tr>
                                    <td class="fw-semibold text-dark">{{ $lista->lista_chamada_id }}</td>
                                    <td>{{ \Illuminate\Support\Str::limit($lista->nome, 40) }}</td>
                                    <td>{{ formatarCpfCnpj($lista->cpf_cnpj) }}</td>
                                    <td>{{ $lista->email }}</td>
                                    <td>{{ formatarTelefone($lista->celular) }}</td>
                                    <td>
                                        {{ $lista->created_at ? \Carbon\Carbon::parse($lista->created_at)->format('d/m/Y H:i') : '—' }}
                                    </td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.adminListaChamada.show', $lista->lista_chamada_id) }}" title="Ver" class="btn btn-sm btn-outline-primary mb-2 mb-md-0 me-md-2">
                ...
                {{-- Paginação --}}
                @if(method_exists($listaChamadas, 'links'))
                    <div class="mt-4 custom-pagination-wrapper">
                        {{ $listaChamadas->links('pagination::bootstrap-5') }}
                    </div>
                @endif
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('listaChamadaFormGerarPdf');
        const modal = document.getElementById('gerarModalPdfListaDeChamada');
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('gerChamadaListaExcelForm');
        const modal = document.getElementById('chamadaListaGerarModalExcel');
