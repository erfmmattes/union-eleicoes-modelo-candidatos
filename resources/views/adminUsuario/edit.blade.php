...
@section('title', 'Union Eleições - Editar Usuário')

@section('content')
<div class="container">
    <div class="mb-5">
        <h1 class="h2 fw-bolder text-dark">Editar Usuário</h1>
        <p class="text-muted">Atualize as informações do usuário selecionado.</p>
    </div>

    <div class="card shadow-lg border-0 rounded-3 stat-card">
        <div class="card-body p-4">
            <form action="{{ route('admin.adminUsuario.update', $usuario->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="tipo_usuario" class="form-select @error('tipo_usuario') is-invalid @enderror" required>
                            ...
                            @foreach ($listTiposUsuarios as $listTiposUsuario)
                                <option value="{{ $listTiposUsuario->slug }}"
                                    {{ old('tipo_usuario', $usuario->tipo_usuario) === $listTiposUsuario->slug ? 'selected' : '' }}>
                                    {{ $listTiposUsuario->nome }}
                                </option>
                            @endforeach
                        </select>
                        @error('tipo_usuario')
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-check form-switch">
                            <input 
                                class="form-check-input cp-tr" 
                                type="checkbox" 
                                role="switch" 
                                name="status" 
                                id="status" 
                                value="1" 
                                {{ old('status', $usuario->status ?? false) ? 'checked' : '' }}
                            >
                        </div>
                    </div>
                </div>
                                    ...
                                    @foreach($acoes as $acao)
                                        <div class="form-check">
                                            <input class="form-check-input cp-tr" type="checkbox"
                                                ...
                                                value="{{ $acao }}"
                                                id="permissao-{{ $tela->slug }}-{{ $acao }}"
                                                {{ in_array($acao, old("permissoes.$tela->slug", array_keys(array_filter($permissoesUsuario)))) ? 'checked' : '' }}>
                                            <label class="form-check-label cp-tr" for="permissao-{{ $tela->slug }}-{{ $acao }}">
