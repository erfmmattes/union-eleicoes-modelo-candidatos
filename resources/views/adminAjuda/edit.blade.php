...
@section('title', 'Union Eleições - Editar Ajuda')

@section('content')
<div class="container">
    <div class="mb-5">
        <h1 class="h2 fw-bolder text-dark">Editar Ajuda: {{ $ajuda->titulo }}</h1>
        <p class="text-muted">Atualize as informações da ajuda selecionada.</p>
    </div>

    <div class="card shadow-lg border-0 rounded-3 stat-card">
        <div class="card-body p-4">
            <form action="{{ route('admin.adminAjuda.update', $ajuda->id) }}" method="POST">
                @csrf
                @method('PUT')
